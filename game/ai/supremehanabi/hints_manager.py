...
class Meaning(dict):
    ...
    def __add__(self, other):
        return Meaning(
            (card_pos, cards | other[card_pos]) for (card_pos, cards) in list(self.items()) if card_pos in other
        )
...
class HintsManager:
    ...
    def compute_information(self, player_id, meanings=None):
        ...
        for (information, meaning) in list(meanings.items()):
            # does this meaning apply?
            if all(hand[card_pos] is None or hand[card_pos] in cards for card_pos, cards in list(meaning.items())):
                # yes
                return information
