...
class BaseHintsManager(object):
    ...
    def is_duplicate(self, card):
        ...
        for (player_id, hand) in list(self.strategy.hands.items()):
...
class ValueHintsManager(BaseHintsManager):
    ...
    def infer_playable_cards(self, player_id, action):
        ...
        if hint_type == Action.NUMBER:
            ...
            visible_colors = set(card.color for (i, hand) in list(self.strategy.hands.items()) for card in hand if i != player_id and card is not None)   # numbers visible by me and by the hinter
        ...
        # the alternative hint would have been on numbers
            visible_numbers = set(card.number for (i, hand) in list(self.strategy.hands.items()) for card in hand if i != player_id and card is not None)   # numbers visible by me and by the hinter
        ...
        involved_cards = [hand[cards_pos[i]] for (i, hand) in list(self.strategy.hands.items()) if i != player_id and i in cards_pos] + [self.strategy.hands[action.player_id][card_pos] for card_pos in action.cards_pos if (action.player_id not in cards_pos or card_pos != cards_pos[action.player_id])]
        ...
        alternative_involved_cards = [hand[alternative_cards_pos[i]] for (i, hand) in list(self.strategy.hands.items()) if i != player_id and i in alternative_cards_pos]
    ...
    def decode_hint(self, player_id, action):
        ...
        # update knowledge
        for (target_id, card_pos) in list(cards_pos.items()):
        ...
        if self.id in cards_pos:
            ...
            involved_cards = [hand[cards_pos[i]] for (i, hand) in list(self.strategy.hands.items()) if i != player_id and i in cards_pos]
    ...
    def compute_hint_value(self, turn, hint_type):
        ...
        modulo = Card.NUM_NUMBERS if hint_type == Action.NUMBER else Card.NUM_COLORS
        involved_cards = [hand[cards_pos[i]] for (i, hand) in list(self.strategy.hands.items()) if i in cards_pos]
    ...
    def get_hint(self):
        ...
        for hint_type in Action.HINT_TYPES:
            ...
            involved_cards = [self.strategy.hands[i][card_pos] for (i, card_pos) in list(cards_pos.items())]
        ...
        possibilities = {a: b for (a,b) in list(possibilities.items()) if b is not None}


        if len(possibilities) > 0:
            score, action = sorted(iter(list(possibilities.values())), key = lambda x: x[0])[-1]
...
class PlayabilityHintsManager(SumBasedHintsManager):
    ...
    def update_knowledge(self, hinter_id, data):
        ...
        for (p_id, hand) in list(self.strategy.hands.items()):
            if p_id == hinter_id:
                # skip the hinter
                continue
...
class CardHintsManager(SumBasedHintsManager):
    ...
    def hint_matching(self, board, kn, hinter_id):
        ...
        if kn.color:
            ...
            while counter < self.modulo(hinter_id) and sum(Card.NUM_NUMBERS - n for n in list(fake_board.values())) > 0:
                # pick next color
